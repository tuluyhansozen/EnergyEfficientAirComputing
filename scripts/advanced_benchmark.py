                    ...
                    for i, user in enumerate(users):
                        if edges:
                            ...
                            user.location = Location(
                                x=edge.location.x + offset_x,
                                y=edge.location.y + offset_y,
                                z=0
                            )
                else:
                    for user in users:
...
"""
        ...
        if valid:
            ...
            most_efficient = min(valid, key=lambda x: x.total_energy if x.total_energy > 0 else float('inf'))
...
"""
    ...
    def _generate_summary(self) -> str:
        ...
        lowest_latency = min(valid, key=lambda x: x.avg_latency if x.avg_latency > 0 else float('inf'))
