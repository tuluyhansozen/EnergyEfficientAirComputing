...
def create_bar_comparison(
    ...
    for bar, val in zip(bars, values):
        ...
        ax.annotate(f'{val:.1%}' if metric == 'success_rate' else f'{val:.1f}',
...
def create_multi_metric_chart(
    ...
    names = [d.get('name', d.get('config_name', 'Unknown'))[:15] for d in data]  # Truncate long names
...
def create_scatter_plot(
    ...
    scatter = ax.scatter(x_values, y_values, c=colors, cmap='RdYlGn', 
                         s=150, edgecolors='black', linewidth=0.5, alpha=0.8)
    
    # Add labels
    for i, name in enumerate(names):
        ax.annotate(name[:12], (x_values[i], y_values[i]),
...
def generate_basic_benchmark_charts(results_dir: Path):
    ...
    create_radar_chart(
        data,
        [('success_rate', 'Success'), ('avg_qos', 'QoS'), 
         ('total_tasks', 'Throughput'), ('total_energy', 'Energy'),
         ('avg_latency', 'Latency')],
...
def generate_advanced_benchmark_charts(results_dir: Path):
    ...
    if valid_data:
        create_heatmap(
            valid_data,
            [('success_rate', 'Success'), ('avg_qos', 'QoS'),
             ('total_energy', 'Energy'), ('avg_latency', 'Latency')],
        ...
        create_radar_chart(
            valid_data[:8],  # Top 8
            [('success_rate', 'Success'), ('avg_qos', 'QoS'),
             ('total_tasks', 'Throughput'), ('avg_latency', 'Latency')],
...
def main():
    ...
    # Basic report charts
    basic_charts = [
        ("Success Rate Comparison", charts_dir / "basic_success_rate.png",
         "Task success rate across different infrastructure configurations."),
        ("Energy Consumption", charts_dir / "basic_energy.png",
         "Total energy consumed by each configuration."),
        ("Multi-Metric Comparison", charts_dir / "basic_metrics.png",
         "Normalized comparison of success rate, QoS, and throughput."),
        ("Energy-Latency Trade-off", charts_dir / "basic_tradeoff.png",
         "Scatter plot showing the relationship between energy and latency."),
        ("Configuration Radar", charts_dir / "basic_radar.png",
    ...
    adv_charts = [
        ("UAV Positioning Success Rates", charts_dir / "adv_uav_positioning_success.png",
         "Comparison of UAV positioning strategies."),
        ("Mobility Pattern Impact", charts_dir / "adv_mobility_patterns_success.png",
         "Success rates under different user mobility patterns."),
        ("Scheduling Algorithm Comparison", charts_dir / "adv_scheduling_success.png",
         "Performance of different scheduling algorithms."),
        ("Performance Heatmap", charts_dir / "adv_heatmap.png",
         "Heatmap showing normalized metrics across all configurations."),
        ("Top Configurations Radar", charts_dir / "adv_radar.png",
