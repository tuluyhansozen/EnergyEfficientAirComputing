...
class Server(ABC):
    ...
    def get_instant_utilization(self, time_interval: float, sim_time: float) -> float:
...
class UAV(Server):
    ...
    def __post_init__(self) -> None:
        ...
        logger.info(
            f"UAV {self.server_id} created at {self.location}, " f"battery={self.battery_level}%"
        )
    ...
    def consume_energy(self, amount: float) -> None:
        ...
        logger.debug(
            f"UAV {self.server_id}: Battery {self.battery_level:.1f}% " f"({self.energy_mode.name})"
        )

    def consume_flight_energy(self, distance: float, velocity: float = 2.5) -> float:
        ...
        logger.info(f"UAV {self.server_id}: Flight {distance:.1f}m consumed {energy:.2f}J")
        return energy
