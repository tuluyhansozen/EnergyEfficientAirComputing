...
class ChargingStation:
    ...
    def start_charging(self, uav: UAV) -> bool:
        ...
        if not self.is_available:
            logger.warning(
                f"Station {self.station_id}: No slots available for UAV {uav.id}"
            )
            return False
        
        if uav.id in self._charging_uavs:
            logger.warning(
                f"Station {self.station_id}: UAV {uav.id} already charging"
            )
            return False
    ...
    def stop_charging(self, uav: UAV) -> bool:
        ...
        if uav.id not in self._charging_uavs:
            logger.warning(
                f"Station {self.station_id}: UAV {uav.id} was not charging"
            )
            return False
...
class ChargingStationRegistry:
    ...
    def find_all_in_range(
        ...
        in_range = [
            ...
            if s.distance_to(location) <= max_distance
        ]
