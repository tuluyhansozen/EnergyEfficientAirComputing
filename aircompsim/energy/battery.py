...
class BatteryModel:
    ...
    def energy_mode(self) -> EnergyMode:
        ...
        return (
            EnergyMode.HIGH
            if self.state_of_charge > 70
            else (
                EnergyMode.MEDIUM
                if self.state_of_charge > 30
                else EnergyMode.LOW if self.state_of_charge > 10 else EnergyMode.CRITICAL
            )
        )
    ...
    def discharge(self, energy_joules: float) -> bool:
        ...
        logger.debug(f"Battery discharged {energy_joules:.1f}J, SoC: {self.state_of_charge:.1f}%")
        return True
    ...
    def charge(self, duration_seconds: float, charge_rate: float = 1.0) -> float:
        ...
        logger.debug(f"Battery charged {actual_energy:.1f}J, SoC: {self.state_of_charge:.1f}%")
    ...
    def can_complete_mission(self, estimated_energy: float, reserve_margin: float = 0.2) -> bool:
    ...
    def estimate_remaining_flight_time(self, power_consumption: float) -> float:
