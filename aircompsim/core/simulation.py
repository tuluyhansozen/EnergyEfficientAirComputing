...
class Simulation:
    ...
    def initialize(self) -> None:
        ...
        cloud_location = Location(
            x=self.boundary.max_x,
            y=self.boundary.max_y,
            z=0
        )
    ...
    def _create_users(self) -> None:
        ...
        for _ in range(self.config.user_count):
            user = User(
                location=self.boundary.random_location(altitude=0)
            )
    ...
    def _select_server(
        ...
        return min(
            candidates,
            key=lambda s: s.get_utilization(self.simulation_time)
        )
    ...
    def _collect_results(self) -> SimulationResults:
        ...
        # Total energy
        total_energy = (
            sum(uav.get_energy_consumption() for uav in UAV.get_all()) +
            sum(edge.get_energy_consumption() for edge in EdgeServer.get_all())
        )
